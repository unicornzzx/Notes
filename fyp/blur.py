...
class Blur():
    ...
    def filter(self, image, mask, template, base, frame_name):
...
if __name__=='__main__':
    ...
    segmentation_base = '/media/labusr/DATA/segmentation/'
    frames_base = '/media/labusr/DATA/frames/'
    processed_base = '/media/labusr/DATA/processed/'
    video_types = os.listdir(segmentation_base)
    for video_type in video_types:
        video_names = os.listdir(segmentation_base + video_type + '/')
        for i in range(len(video_names)):
            time1 = time.time()
            p = Pool() #use multi processing to speed up the program
            ...
            for j in range(len(mask_names)):
                ...
                frame = frames_base + '/' + video_type + '/' + video_name_f + '/' + frame_name
                mask = blur.getMask(segmentation_base + video_type + '/' + video_name + '/' + mask_name)
                new = processed_base + '/' + video_type + '/' + video_name_new + '/'
                p.apply_async(blur.filter, args=(frame,mask,temp,new,frame_name))
                #print('generating---'+ new + frame_name)
            p.close()
            p.join()
            print('All images in: '+video_name+' are produced.')
            time2 = time.time()
            print('number of frames: '+ str(len(mask_names))  + 'time: ' + str(time2-time1))
